...
def _format_content(value, renderer):
    """A legacy or unrenderable optional format never hides the stored transcript."""
    try:
        return renderer(value)
    except (ValueError, TypeError, KeyError):
        return h("p", {"class_": "muted sl-research-format-unavailable"}, t("cf_format_unavailable"))
...
def register_councils(app) -> None:
    ...
    def council_detail(session_id: str) -> str:
        ...
        h2h_html = (_format_content(session["head_to_head"], head_to_head_content) if is_h2h else "")
